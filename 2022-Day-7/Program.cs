...
namespace _2022_Day_7
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 1; i < input.Length; i++)
            {
                ...
                foreach (var response in contents)
                {
                    if (response != "")
                    {
                        temp.Add(response.Split('\r')[0].Trim());
                    }
                }
            ...
            for (int i = 0; i < commandsList.Count; i++)
            {
                if (commandsList[i].Item1 == "ls")
                {
                    ...
                    {
                        if (long.TryParse(commandsList[i].Item2[j].Split(' ')[0], out working))
                        {
                            tot += working;
                        }
                        else
                        {
                            children.Add(commandsList[i].Item2[j].Split(' ')[1].Trim());
                        }
                    }
            ...
            long tempTot;
            foreach (var key in dirChildren.Keys)
            {
                dirSizes[key] = GetDirSize(dirSizes, dirChildren, key);
            }
            ...
            {
                if (dirSizes[key] <= 100000) total += dirSizes[key];
            }
            ...
            foreach (var key in dirSizes.Keys)
            {
                if (someValue + dirSizes[key] > 30000000 && dirSizes[key] < smallest)
                {
                    smallest = dirSizes[key];
                }
            }
            // Part 2
        ...
        public static string ParseDirChain(string current, List<string> chain)
        {
            ...
            for (int i = 0; i < stackOfThings.Count; i++)
            {
                if (stackOfThings[i] != "")
                {
                    result += stackOfThings[i].Trim();
                    result += "/";
                }
            }
